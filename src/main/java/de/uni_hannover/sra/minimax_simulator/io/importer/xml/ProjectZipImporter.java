...
public class ProjectZipImporter implements ProjectImporter
{
			...
			ProjectImportException
	{
		...
		try
		{
			return new MachineXmlImporter().loadMachine(IOUtils.toBufferedStream(machineStream));
		}
		finally
		{
	...
	private ProjectConfiguration getProjectConfiguration(ZipFile zip) throws IOException,
			ProjectImportException
	{
		ZipEntry userEntry = zip.getEntry("user.xml");
		if (userEntry == null)
			throw new FileNotFoundException("Missing user.xml in project archive");

		InputStream userStream = zip.getInputStream(userEntry);
		try
		{
			return new UserXmlImporter().loadProjectConfiguration(IOUtils.toBufferedStream(userStream));
		}
		finally
		{
	...
	private SignalTable getSignalTable(ZipFile zip) throws IOException,
			ProjectImportException
	{
		ZipEntry signalEntry = zip.getEntry("signal.xml");
		if (signalEntry == null)
			throw new FileNotFoundException("Missing signal.xml in project archive");

		InputStream userStream = zip.getInputStream(signalEntry);
		try
		{
			return new SignalXmlImporter().loadSignalTable(IOUtils.toBufferedStream(userStream));
		}
		finally
		{
			IOUtils.closeQuietly(userStream);
